...
namespace Henriqueak\LaravelBoleto\Boleto;

use Carbon\Carbon;
use Henriqueak\LaravelBoleto\Boleto\Render\Html;
use Henriqueak\LaravelBoleto\Boleto\Render\Pdf;
use Henriqueak\LaravelBoleto\Contracts\Pessoa as PessoaContract;
 ...
 *
 * @package Henriqueak\LaravelBoleto\Boleto
 */
abstract class AbstractBoleto implements BoletoContract
{
    ...
    {
        if (count($instrucoes_impressao) > 6) {
            throw new \Exception('Máximo de 6 instruções.');
        }
    ...
    public function getInstrucoesImpressao()
    {
        if (!empty($this->instrucoes_impressao))
            return array_slice((array)$this->instrucoes_impressao + [null, null, null, null, null, null], 0, 6);
        else
            return [];
    }
    ...
    {
        if (count($this->getDescricaoDemonstrativo()) > 6) {
            throw new \Exception('Atingido o máximo de 6 demonstrativos.');
        }
    ...
    /**
     * Define um array com a descrição do demonstrativo (máximo 6)
     *
    ...
    {
        if (count($descricaoDemonstrativo) > 6) {
            throw new \Exception('Máximo de 6 demonstrativos.');
        }
    ...
    {
        return array_slice((array)$this->descricaoDemonstrativo + [null, null, null, null, null, null], 0, 6);
    }
