...
namespace Henriqueak\LaravelBoleto\Boleto\Render;

use Henriqueak\LaravelBoleto\Contracts\Boleto\Boleto as BoletoContract;
use Henriqueak\LaravelBoleto\Contracts\Boleto\Render\Pdf as PdfContract;
...
class Pdf extends AbstractPdf implements PdfContract
{
    ...
    protected function Bottom($i)
    {
        ...
        $this->SetFont($this->PadraoFont, 'B', $this->fcel);
        $beforeAdressX  = $this->GetX();
        $beforeAddressY = $this->GetY();
        $this->MultiCell(120, $this->cell, $this->_($this->boleto[$i]->getBeneficiario()->getNomeDocumento()."\n".$this->boleto[$i]->getBeneficiario()->getEndereco().' '.$this->boleto[$i]->getBeneficiario()->getCepCidadeUf()),'LR');
        $this->setXY($beforeAdressX + 120, $beforeAddressY);
        $this->Cell(50, $this->cell, null, 'R', 1, 'R');
        $this->setXY($beforeAdressX + 120,$beforeAddressY + $this->cell);
